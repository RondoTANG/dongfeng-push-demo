...
> **本期目标**：实现微信代公众号扫码授权、凭证维护、群发及发布事件实时捕获、文章网页公开元数据去重抓取并入库。
...
| **群发与发布内容感知** | 公众号运营后台执行群发或正常发布，系统通过 Webhook 毫秒级感知，同时支持官方 API 获取已发布文章。 | **V1.0 (本期)** |
| **文章详情抓取与去重** | 群发文章提取 URL 进行网页抓取；**已发布文章**则直接通过调用微信官方 API（freepublish/batchget）获取结构化元数据，**无需走爬虫**。 | **V1.0 (本期)** |
...
*   **V1.0（本期核心：授权接入与内容感知）**
    ...
    *   **事件网关接收**：部署 Webhook 接口接收 `MASSSENDJOBFINISH`（群发）与 `PUBLISHJOBFINISH`（发布）推送，5 秒内幂等响应。
...
* **多公众号统一感知**：微信第三方平台允许配置统一的“消息与事件接收URL”。当任何已授权的子品牌公众号群发或发布文章时，微信均会推送 `MASSSENDJOBFINISH` 到该统一地址，从而实现一个后台对所有品牌的全局感知。
...
```mermaid
graph LR
    A["公众号运营者<br/>微信后台群发/发布"] -->|群发/发布完成| B["微信官方服务器"]
    B -->|MASSSENDJOBFINISH / PUBLISHJOBFINISH<br/>XML 推送| C["护卫军事件网关<br/>(Webhook)"]
...
系统采用**被动感知架构**：在微信第三方开放平台配置 Webhook 事件接收网关。微信服务器在群发/发布完成后主动向我方推送 `MASSSENDJOBFINISH` 事件，处理引擎仅需轻量级 XML 解析即可完成新内容感知。

*   **监控 0 盲区**：100% 覆盖微信公众号最为核心的“群发（Mass Send）”与“发布（Publish）”场景。
*   **感知 0 延迟**：群发/发布完成即时推送，毫秒级流转至监控引擎。
...
### 4.2 群发与发布事件接收 — MASSSENDJOBFINISH / PUBLISHJOBFINISH
...
对于群发（MASSSENDJOBFINISH）事件，由于微信未提供根据 URL 查询文章详情的直接 API，系统接收到 `ArticleUrl` 后：
...
| 权限集 | 作用 | 必要性 |
|--------|------|--------|
| **群发权限集** | 接收 `MASSSENDJOBFINISH` 与 `PUBLISHJOBFINISH` 事件推送 | 🔴 必需 |
...
凭证维护：
  ...
  ↓
事件捕获：
  Webhook 接收 MASSSENDJOBFINISH / PUBLISHJOBFINISH → 解析 ArticleUrl + MsgID + ArticleIdx
  ↓
去重校验：
...
| 模块 | 优先级 | 说明 | 规划版本 |
|------|--------|------|---------|
| **Webhook 事件网关** | P0 | 部署并运行接收微信 `MASSSENDJOBFINISH` / `PUBLISHJOBFINISH` 推送的 HTTP 网关服务 | **V1.0 (本期)** |
