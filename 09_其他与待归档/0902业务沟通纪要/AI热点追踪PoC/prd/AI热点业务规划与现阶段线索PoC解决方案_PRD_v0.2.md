...
| 业务目标 | 识别具体平台正在快速发酵的东风相关事件，并分别判断：哪些事件值得发起原创增长作业；哪些热点源文章或视频值得直接组织点赞、评论等加热作业 |
| 本阶段目标 | 在尚未具备稳定热点数据源的前提下，验证公开信息线索发现、证据核验、事件研判，以及“原创增长”“源内容加热”两类作业草案的生成与审批；本阶段达标不等于具备真实热点监测能力 |
...
第二个问题的加热对象是**热点事件中的源文章或源视频本身**，不是“先完成原创发布，再依据原创投稿效果决定是否二次加热”。原创发布后的效果追踪属于后续运营反馈能力，不作为本次第二条业务链路的前置条件。
...
| 反馈 | 两类作业执行后是否达到业务目标 | 后续回收作业结果，用于评估规则；不作为本轮PoC交付范围 |
...
| 阶段 | 数据基础 | 解决的问题 | 输出 |
|---|---|---|---|
| 公开信息线索与作业联动PoC | 豆包、Codex公开搜索及可访问网页 | 公开线索能否被清洗、聚合和研判；运营确认后，能否分别形成有证据的原创增长草案和绑定具体源文章／视频的加热草案 | 相关事件线索、品牌内容机会、人工复核项、原创增长草案、源内容加热草案 |
| 真实热点能力 | 稳定的平台原生数据或专业舆情数据源 | 哪个平台、哪个事件正在快速发酵，是否达到响应时机 | 分平台热点状态、增速、风险等级、响应建议 |

两阶段共用来源标准化、事件聚合、品牌关联、风险判断、双路任务草案和运营审核能力。正式分发、执行结果与效果跟踪在后续业务系统联动阶段建设。
...
阶段一对外称为“公开信息线索与作业联动PoC”。达标表示线索研判、原创增长草案、热点源内容加热草案和分别审批的链路可行；不表示已经具备全平台持续发现或真实热点判定能力。

## 1.6 本阶段SMART目标

| 维度 | 要求 |
|---|---|
| 具体 | 对9个业务品牌及8个行业主题执行公开搜索，完成来源核验、事件聚合、品牌关联、风险提示，并对运营确认的事件分别生成原创增长草案和绑定源文章／视频的加热草案 |
...
| 相关 | 验证业务终局中的信息发现、事件研判，以及“原创增长”“源内容加热”两类任务草案链路，并量化真实热点数据源缺口 |
...
草案通过审批后也不自动下发。原创内容发布后的链接回填、指标快照和二次加热属于后续效果反馈能力，不进入本轮范围。
...
| 配置管理 | 品牌、来源、查询、过滤、聚合、风险、热点准入、双路草案和互动动作规则 |
| 无效日志 | 自动过滤规则、来源、恢复操作和抽检结果 |
| 双路作业草案与审批 | 原创增长方向，或绑定目标文章／视频的加热动作、目标平台、用户标签、证据、风险和审批 |
...
- 汇总已核验证据、事实冲突和证据缺口。
- 生成关联补采查询、原创增长草案和绑定目标来源的源内容加热草案。
- 对文本风险提供标签和理由。
...
| 作业执行结果 | 本轮不实施 | 后续复用现有非微信平台采集并补充微信系方案，用于运营效果反馈 |
...
| `task_draft` | 待审批任务草案；以`draft_purpose`区分原创增长和源内容加热 |
| `task_draft.target_source_id` | 源内容加热草案绑定的事件来源；原创增长为空 |
| `task_draft.engagement_actions` | 源内容加热的点赞、正向评论、转发或收藏动作 |
...
| `POST /api/events/{event_id}/review` | 保存事件结论、行动方向和源内容目标，并生成一类或两类草案 | PoC |
| `GET /api/drafts` | 按行动方向和审批状态查询草案 | PoC |
| `PATCH /api/drafts/{task_draft_id}` | 修改草案表达、平台、标签或互动动作 | PoC |
| `POST /api/drafts/{task_draft_id}/review` | 分别审批原创增长或源内容加热草案 | PoC |
...
完成标准：形成查询覆盖、线索漏检、自动过滤、事件聚合、候选质量、双路草案质量和人工耗时基线；不输出热点识别准确率。
...
任一运行、候选和双路草案均能追溯到当时使用的品牌、查询、过滤、风险和任务规则配置版本。
...
- 下游影响：原创增长草案、源内容加热草案、运营审核、平台标签分发和后续作业执行结果。
