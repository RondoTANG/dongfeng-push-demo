...
| 业务目标 | 识别具体平台正在快速发酵的东风相关事件，并分别判断：哪些事件值得发起原创增长作业；哪些热点源文章或视频值得直接加热；原创发布后是否出现传播增长、是否需要进一步组织二次加热 |
| 本阶段目标 | 在尚未具备稳定热点数据源的前提下，验证公开信息线索发现、证据核验、事件研判、两类初始作业草案，以及原创发布结果登记、指标快照、增长观察和原创后二次加热草案；本阶段达标不等于具备真实热点监测能力 |
...
业务要求系统回答两个核心问题，并把原创增长继续闭环：

1. 哪个平台、哪个东风相关事件正在快速发酵，其中哪些热点值得发起原创作业，借势形成内容增长。
2. 对已经发现的热点，哪一篇源文章或哪一个源视频值得直接加热，以及应组织点赞、正向评论等哪类加热作业。

第二个问题的加热对象是**热点事件中的源文章或源视频本身**，不等待护卫军用户先发布原创。同时，第一条原创增长路径不能在发布后结束：原创内容发布并回填链接后，还要采集同一内容的后效指标快照，观察是否出现传播增长，并由运营判断是否需要生成一份独立的“原创后二次加热”草案。

因此，业务结构是**两个入口、一个后效分支**：热点源内容加热与原创后二次加热目标不同、触发条件不同、草案类型不同，不能合并。
...
| 决策 | 事件是否与东风相关、是否值得响应；适合原创增长还是直接加热源内容 | 品牌关系、实体存疑原因、事实证据、风险提示、行动方向和运营复核 |
| 执行A：原创增长 | 哪些事件值得形成原创内容，在哪个平台表达 | 原创评论／原创内容草案、建议平台、平台能力标签匹配和运营审批 |
| 执行B：源内容加热 | 哪条热点源文章或视频值得直接放大，采取什么互动动作 | 目标来源选择、目标链接、点赞／正向评论等动作、时间窗、目标人群和运营审批 |
| 原创后效反馈 | 原创发布后是否产生传播增长，是否需要进一步加热该原创内容 | 回填原创链接、接收两个以上指标快照、计算同口径增量、运营判断、生成原创后二次加热草案并独立审批 |
...
→ 东风关联与风险研判
→ 运营确认行动方向
  ├─ 原创增长：生成原创评论／内容草案→运营审批→正式发布→回填原创链接
  │  → 后效指标快照→增长观察→运营判断是否生成原创后二次加热草案→独立审批
  └─ 源内容加热：选择目标文章／视频→生成点赞／正向评论等加热草案→运营审批
...
| 阶段 | 数据基础 | 解决的问题 | 输出 |
|---|---|---|---|
| 公开信息线索与作业联动PoC | 豆包、Codex公开搜索、可访问网页，以及已发布原创链接的人工／业务系统指标回传 | 公开线索能否被清洗、聚合和研判；能否形成两类初始草案；原创发布后能否记录快照、观察增量并生成独立二次加热草案 | 事件线索、原创增长草案、源内容加热草案、原创发布记录、后效快照、增长判断、原创后二次加热草案 |
...
3. **源内容加热必须绑定具体目标。**草案必须引用当前事件证据中的一篇文章或一个视频，记录平台、内容链接和互动动作；官网、普通网页、平台不明或无有效链接的来源不能生成加热草案。
4. **原创后二次加热必须由发布后效触发。**必须先有已审批原创草案、实际发布链接和至少两个同口径指标快照；系统只计算增量，不用跨平台统一阈值自动决定是否加热，运营结论和二次加热草案均需留痕并单独审批。
...
| 对热点源内容组织加热 | 选择热点事件中的具体文章或视频，组织点赞、正向评论等互动 | 运营从事件证据中选择一个可执行平台链接，系统生成绑定目标内容、平台、动作和风控说明的加热草案 | **可以验证草案与审批链路，不能自动判定该内容真实热或值得加热**；目标有效性和业务价值由运营确认，草案不得自动发布 |
...
阶段一对外称为“公开信息线索与作业联动PoC”。达标表示线索研判、两类初始作业草案，以及原创发布后效追踪与二次加热草案链路可行；不表示已经具备全平台持续发现或真实热点判定能力。

## 1.6 本阶段SMART目标

| 维度 | 要求 |
|---|---|
| 具体 | 对9个业务品牌及8个行业主题执行公开搜索，完成来源核验、事件聚合、品牌关联、风险提示，生成两类初始草案；对已发布原创登记链接、接收至少两个指标快照并形成后效判断和可选二次加热草案 |
...
| 相关 | 验证信息发现、事件研判、原创增长、热点源内容加热，以及原创发布后效追踪和二次加热闭环，并量化真实热点数据源与平台效果采集缺口 |
| 有时限 | 10个工作日完成：配置冻结1天、本地每3小时运行并连续观察7天、数据汇总2天；不承诺生产级7×24运行保障 |
...
- 决定通过、修改后通过、驳回或继续观察。
- 分别审批原创增长、热点源内容加热和原创后二次加热草案。
- 查看原创发布后的指标增量，决定继续观察、无需加热、人工核验或生成二次加热草案。

### 内容运营

- 补充官方事实、素材和允许使用的表达。
- 选择“原创增长”或“源内容加热”行动方向；源内容加热时必须指定目标文章或视频。
- 确认目标平台和禁用表述。
...
flowchart TD
    ...
    M --> N
    N --> O{运营选择行动方向}
    O -->|原创增长| P[生成原创评论或内容草案]
    O -->|源内容加热| Q[选择事件中的目标文章或视频]
    Q --> R[生成点赞、正向评论等加热草案]
    P --> S[分别人工审批]
    R --> S
    S --> T[后续接入护卫军作业系统]
...
事件通过运营审核后，运营可以选择一个或两个行动方向：

- **原创增长**：生成原创评论或原创内容草案，用于借势形成内容增长。
- **源内容加热**：直接对热点事件中的一篇文章或一个视频生成互动草案，不要求用户另发原创内容。

两类草案共用事件ID、行动方向、任务类型、任务标题、事实背景、建议平台、目标用户标签、证据来源、风险说明和禁止表述。源内容加热草案还必须包含目标来源ID、目标URL、目标内容标题和互动动作。

同一事件可生成一份原创增长草案，并可对最多三条不同目标来源分别生成加热草案。两类草案分别审批。草案只能引用已经核验的事实。PoC阶段仅保存`draft_pending_review`，不自动创建正式作业、不自动发布、不自动触达用户。
...
草案通过审批后也不自动下发。热点源内容加热不等待原创发布；原创发布后的二次加热由F16—F18独立触发。
...
| 配置管理 | 品牌、来源、查询、过滤、聚合、风险、热点准入、初始双路草案、后效快照和互动动作规则 |
...
| `POST /api/events/{event_id}/review` | 保存事件结论、两类初始行动方向和源内容目标，并生成一类或两类初始草案 | PoC |
...
- 本地每3小时触发17条基础查询和双路公开搜索。
- 对有效行业事件执行9品牌关系验证。
- 生成任务草案但不下发。
- 运营标注通过、修改后通过、驳回和观察。
- 对运营确认的事件分别生成原创增长草案和源内容加热草案，并独立审批。
- 对已审批且实际发布的原创登记链接，接收至少两个同口径指标快照，展示增量并由运营决定继续观察、无需加热、人工复核或生成二次加热草案。

完成标准：形成查询覆盖、线索漏检、自动过滤、事件聚合、候选质量、初始双路草案质量、原创后效快照与二次加热链路及人工耗时基线；不输出热点识别准确率，不自动执行点赞或评论。
...
- 将已审核的事件与两类作业候选交给护卫军；由护卫军按业务规则创建正式原创增长或源内容加热作业。
...
同一事件选择“原创增长”和“源内容加热”后，系统分别创建两份草案，具有不同`draft_purpose`，且审批状态互不影响。
