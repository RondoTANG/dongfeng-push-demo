...
PoC要求AI承担语义研判与作业内容生成；下面先区分当前已运行节点与必须补齐的AI任务。自动处理不等于AI处理。

### 3.1.1 PoC逐节点执行分工与当前接通状态

“必须接入”表示PoC目标能力，不表示当前服务已实现。AI分析与生成可以由受控模型调用完成；需要搜索工具和多步核验的任务由Agent执行。两者都必须留下可追溯的执行记录。
...
| 事件语义研判 | AI分析｜必须接入，尚未接通 | 当前使用标题规范化、品牌与日期组合聚合；车型正则和风险关键词打标。不是语义聚类或逐事件AI分析。 | AI核对同一事件、主体／动作／对象、品牌与实体关系、事实冲突、风险语境、作业价值及存疑原因，逐项引用证据；程序校验后形成待审核建议。 |
| 热点数据检查 | 程序规则 | 搜索事件由程序输出热点不可判定及缺失原因。 | AI不得以搜索排名、条数或主观评分替代平台指标。 |
| 定向补证 | 运营＋Agent／接口＋程序 | 运营确认问题、查询词、工具与次数；Codex侧由Agent检索，豆包侧直接调API；证据回存后重新待审核，未自动判断疑点是否解决。 | AI必须对照原疑点输出已解决／未解决、依据与剩余缺口；该回答核验尚未接通。没有明确问题不发起补证。 |
| 事件审核 | 运营＋程序校验 | 人工通过／驳回；通过后确定事实结论和行动方向，程序检查前置条件。 | AI只提供建议，不代审核；通过后才进入草案生成。 |
| 平台适配与作业内容生成 | AI分析生成｜必须接入，尚未接通 | 当前标题关键词推荐平台、截取证据并拼接固定章节模板；没有草案生成模型调用。 | AI依据已审核事实、行动方向、可用素材和平台规范，生成核心命题、创作方向、分平台要求及推荐理由。话题无依据时待运营确认；固定作业规则由程序附加。 |
| 草案校验与审批 | 程序规则＋运营 | 程序检查状态、证据及目标绑定；运营编辑并分别审批；不正式下发。 | AI结果必须经结构、来源引用与禁止项检查；不通过则留失败原因，不能把模板兜底记为AI生成成功。 |
| 原创后效与追加加热 | 程序规则＋运营 | 登记实际发布链接、接收快照、计算同口径增量；运营决定是否追加加热，当前草案仍为模板。 | 数值计算不交AI；批准追加后，作业内容复用AI生成节点，绑定实际发布内容而非事件源链接。 |

### 3.1.2 AI任务输入、输出与验收

- **事件研判任务**：输入通过基础准入的来源及证据ID；输出事件归组建议、实体及品牌关系、事实摘要、风险理由、行动建议和不确定原因。程序决定归组写入与状态，不允许AI直接修改审核结果。当前分析工作项的领取／回传接口不等于已接通自动分析消费者。
...
- **调用失败**：AI任务标记失败并允许受控重试；人工继续处理或模板兜底必须明确标记，不能记录为AI分析成功。最终审批继续由运营负责。
...
- 当前Codex搜索仅接收代码构造的检索指令、查询词和搜索结果Schema，不读取这两份完整YAML。应按任务提供必要规则，不让Agent自行决定是否执行审批、删除数据或调整时间窗。
