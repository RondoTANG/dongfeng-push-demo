...
| 决策 | 事件是否与东风相关、是否值得通过原创响应 | 品牌关系、实体存疑原因、事实证据、风险提示和原创价值复核 |
...
| 补充场景：关联源内容直接加热 | 事件中是否已有值得放大的平台文章或视频 | 目标来源选择、目标链接、点赞／正向评论等动作、时间窗、目标人群和运营审批 |
...
| 有时限 | 10个工作日完成：配置冻结1天、由管理员按测试计划手工触发双路运行并连续观察7天、数据汇总2天；不配置定时采集，不承诺生产级7×24运行保障 |
...
9. 输出字段、枚举和数据保留规则。
10. 自动采集启停状态与采集周期；默认停止，仅管理员可修改，周期支持1—168小时。

品牌、查询、来源平台和域名识别规则必须由管理员在配置管理页面增删改并写入配置源；页面不是只读说明。自动采集属于运行时配置，单独持久化启停状态、周期、下次执行时间和最近执行结果。每次变更写入审计。新运行冻结当时配置快照，历史运行不得被后续配置修改回写。
...
- 9条品牌基础查询。
- 8条行业主题查询。
- 每条基础查询分别由豆包和Codex执行，共34个基础搜索任务。品牌或查询被新增、修改、停用后，实际数量按“当前启用查询数×启用采集器数”动态计算并在执行前展示，不再依赖固定17／34常量。

运行前生成查询覆盖清单。运行后记录每个任务的成功、失败、无结果和未执行状态。未执行不得解释为无结果。

所有搜索结果均先使用标题和正文核对9个启用品牌，再进入业务工作台。行业查询只扩大召回，不放宽业务关联条件。当前内容没有目标品牌关联证据的其他品牌销量、上市、召回等资讯记为INV007，保留原始记录，不进入业务有效列表和正常事件队列。存在简称歧义时记录INV009及具体待核验原因，不把不确定项标为有效。

关联证据必须来自当前内容，不能使用搜索词、推荐区、版权栏、官方域名或“同属汽车行业”代替。含有竞品不等于无效：正文明确涉及东风目标品牌的比较、合作或行业影响仍可进入事件研判。新车型等事实存疑不要求建立全量实体字典，仍按事件核验。

基础阶段不自动为每条行业结果追加9品牌双路收费补采。需要补证时先明确问题、查询词、工具与预计调用次数，经人工确认再执行。当前确定性筛选可能漏掉未提及目标品牌的间接行业机会，未核实前不伪造其关联。

### F03 公开搜索接入

豆包与Codex都执行当前启用的同一份基础查询目录。豆包用于公开网页召回，Codex使用运行机器上的登录态执行公开搜索、补充召回并核验可访问页面。默认目录为17条，因此默认完整运行形成17条查询×2个工具＝34个独立任务；快速验证形成1条查询×2个工具＝2个独立任务。两个工具分别记录请求状态、搜索时间、查询词、结果顺序和原始响应引用。

搜索排序、搜索结果数和供应商权威等级只作为原始搜索字段保留，不得进入热度计算。

单一搜索工具失败时继续执行另一工具并将批次标记为部分成功；两个工具均失败时标记为失败。服务端对手工完整运行设置3小时冷却、快速验证设置10分钟冷却；重复点击直接返回剩余等待时间。管理员可另行设置自动采集周期并启停，默认停止；周期范围1—168小时，首次开启或修改周期后等待一个完整周期再运行，避免启动服务立即产生收费调用。自动任务到期时若已有采集运行则暂不并发，后续轮询再执行。访客既不能修改自动采集配置，也不能触发搜索。
...
同一批次内同一规范化URL只保留一条`source_item`，但每一次由哪个工具、哪个查询、何时搜索发现，必须分别写入`source_discovery`。工作台独立展示来源平台、搜索来源（豆包／Codex，多路命中全部显示）、正文／搜索摘要、内容发布时间和搜索获取时间。详情顶部逐条显示工具、完整查询词及搜索时间，下方展示品牌关联依据、正文及日期依据。支持按两类时间分别筛选。

来源记录增加`business_relation`，包含eligible、status、rule_id、reason与relations；关联项记录品牌ID、名称、命中词及证据摘录。工作台查询、事件聚合与草案生成前均检查业务关联，不依赖前端隐藏数据。相同URL后续获得更完整且通过清洗的搜索证据时允许重新判定，保留各次发现记录及变更审计。

事件审核桌面端采用“固定队列头＋中间列表滚动＋底部分页”的双栏布局，详情区独立滚动。手机端不压缩双栏：先只展示可自然滚动的事件列表，点击事件进入独立详情视图，详情使用页面自然滚动并提供固定可识别的返回列表入口；不得在手机端保留列表内部滚动后再把详情接在下方。窗口缩小或缩放后分页不得被挤出可见区域。

有效线索采用采集时刻前72小时的发布时间窗口，优先最近24小时；正文头部明确的发布元信息优先于供应商日期，并保留冲突记录。不把正文叙事日期、索引更新或搜索时间当成发布时间。超出窗口记为旧闻；发布时间无法核验或位于未来时写入排除记录，均不进入有效线索及事件审核。旧事件的新回应必须有独立近期内容证据，不能直接放行旧页面。

风险标签从`清洗聚合与判定规则_v0.1.yaml`的`risk_gate.rules`统一加载。配置管理展示中文名称、关键词、核验要求和启用状态；事件与草案使用同一份中文映射。当前标签包括销量与交付数据、价格与优惠、事故与安全、产品召回、监管与政策、竞品对比、智能驾驶能力表述、争议或负面线索、事实冲突。关键词命中只提示核验；事实冲突由证据核验或人工确认触发。修改配置文件后重新加载生效。
...
- 纯经销商报价、优惠和到店引流。
- 可核验发布时间超过采集时刻前72小时的旧页面；新回应需由独立近期内容证明。
- 不能核验发布时间或发布时间位于未来的内容，暂不纳入有效线索并说明原因。
...
- 系统设管理员和只读访客两种访问角色。管理员可执行采集、审核、配置及访问密钥管理；访客只可查看已授权页面与数据。
- 管理员在后台生成访客密钥时填写名称和可选失效时间；生成后可直接复制，也可稍后在管理页重新查看。重新查看必须再次输入管理员密钥，并记录查看人、时间和目标密钥编号。服务端同时保存登录校验用的加盐哈希与服务器主密钥加密后的密文，不保存可直接读取的明文；列表仅展示末四位。
- 访客登录后不显示“访问密钥管理”模块；即使绕过页面直接请求，服务端仍须返回无权限。访客对任何写接口均无权限，避免共享地址后产生搜索费用或修改业务数据。
- 撤销访客密钥后，该密钥创建的会话立即失效。管理员密钥由部署环境变量管理，不在页面回显、不入数据库、不入Git。
- 页面适配桌面、平板和手机：手机端使用抽屉式导航、单列内容、适配宽度的弹窗和可横向滚动的数据表；事件审核采用列表页与详情页分屏，两个视图均由页面自然滚动，不得依赖固定桌面宽度或嵌套滚动。
- 部署后自动采集默认停止；管理员可按测试计划设置1—168小时周期并启停。完整运行与定时运行共用3小时冷却和审计，快速验证保留10分钟冷却；定向补证仍只允许逐次人工确认。
...
- 系统不配置定时采集；服务启动不会自动执行搜索。
- 只有管理员可触发运行；访客页面操作和直接接口请求均被拒绝。
...
PoC要求AI承担语义研判与作业内容生成。当前服务已由运行机器上的Codex CLI执行真实模型调用；自动处理不等于AI处理，下面逐节点区分模型任务、程序规则和人工审核。

### 3.1.1 PoC逐节点执行分工与当前接通状态

AI分析与生成由受控Codex CLI调用完成；需要公开检索的任务使用Codex搜索能力，针对已入库证据的语义研判不再联网。两类调用都保留工作项、执行器、模型、输入证据、结构化输出和失败原因。

| 流程节点 | 责任主体 | 当前真实执行 | PoC规定处理方式 |
|---|---|---|---|
| 运行与任务编排 | 程序规则＋运营 | 管理员手工触发或开启周期采集；服务按启用查询动态生成双路任务，控制冷却、幂等与失败记录；不调用AI。 | 保持程序执行，Agent不得绕过权限、冷却或重复运行。 |
| 豆包公开搜索 | 程序调用外部接口 | Python调用豆包搜索API，整理返回字段；本项目未追加AI分析调用。 | 属于检索工具调用，不视为事件研判。 |
| Codex公开搜索 | AI Agent｜已接通 | 接收查询词与时间范围，使用公开搜索，整理标题、URL、摘要、公开日期并按结构返回。 | Agent执行检索与信息整理；不判断真实热度或替运营审批。 |
| 来源清洗与品牌准入 | 程序规则｜未调用AI | URL／日期解析、72小时窗口、品牌别名和上下文词匹配、栏目与促销过滤。输入可能来自AI搜索，但此节点没有再次调用AI。 | 基础准入保持规则执行；复杂实体关系交下一节点研判，不让模型放宽时间或业务范围。 |
| 事件语义研判 | AI分析｜已接通 | 来源基础归组后，服务自动调用Codex读取事件及来源证据，输出事实摘要、内容语气、实体识别与存疑、风险、行动建议和草案蓝图；程序限制品牌、风险标签、来源引用和平台枚举。 | AI建议只进入待审核，不能自动通过事件、改变真实热点结论或放宽基础准入。 |
| 热点数据检查 | 程序规则 | 搜索事件由程序输出热点不可判定及缺失原因。 | AI不得以搜索排名、条数或主观评分替代平台指标。 |
| 定向补证 | 运营＋Agent／接口＋程序＋AI分析｜已接通 | 运营确认问题、查询词、工具与次数；Codex侧由Agent检索，豆包侧直接调API；证据回存后自动新建AI研判工作项，对照原疑点输出已解决／未解决及剩余缺口，再回到待审核。 | 没有明确问题不发起补证；搜索成功不等于疑点已解决，AI也不得自动通过。 |
| 事件审核 | 运营＋程序校验 | 人工通过／驳回；通过后确定事实结论和行动方向，程序检查前置条件。 | AI只提供建议，不代审核；通过后才进入草案生成。 |
| 平台适配与作业内容生成 | AI分析生成｜已接通 | 事件AI研判同步形成原创增长及各来源加热蓝图；运营通过事件并选择行动后，程序基于对应蓝图生成完整草案。原创后效决定追加加热时另行调用Codex生成针对该发布内容的蓝图。 | AI生成核心命题、创作方向、分平台要求、互动方向及推荐理由；话题无依据时留空待运营确认，固定作业规则由程序附加。 |
| 草案校验与审批 | 程序规则＋运营 | 程序检查状态、证据及目标绑定；运营编辑并分别审批；不正式下发。 | AI结果必须经结构、来源引用与禁止项检查；不通过则留失败原因，不能把模板兜底记为AI生成成功。 |
| 原创后效与追加加热 | 程序规则＋运营＋AI生成｜已接通 | 程序登记实际发布链接、接收快照并计算同口径增量；运营决定追加加热后，Codex依据目标平台、原创内容和增量结论生成个性化加热要求。 | 数值计算不交AI；追加草案绑定实际发布内容而非事件源链接，并继续人工审批。 |

### 3.1.2 AI任务输入、输出与验收

- **事件研判任务**：输入通过基础准入的来源及证据ID；服务在聚合后自动调用Codex，输出实体、事实摘要、内容语气、风险理由、行动建议、不确定原因和作业蓝图。程序决定写入与状态，不允许AI直接修改审核结果。
- **补证任务**：输入明确疑点及已批准的查询范围；输出每个疑点的解决情况、证据和剩余问题。搜索成功不等于补证问题已解决。
- **作业生成任务**：输入已通过的事件结论、审核选择的行动方向、证据、可用素材与平台规范；输出个性化作业要求、推荐平台与理由、创作方向、需运营补齐项。加热任务须绑定具体源内容或实际原创发布链接，不能混用。
- **验收证据**：每次AI分析／生成须保存执行ID、关联事件／草案ID、模型或执行器、提示词与规则版本、输入证据引用、原始输出、校验结果和失败原因。不能以存在YAML、返回接口或固定模板证明AI任务已执行。
- **调用失败**：AI任务标记失败并允许在事件详情受控重试；AI未完成时不能通过事件并生成草案，不使用固定模板伪装成功。最终审批继续由运营负责。

### 3.1.3 YAML与执行服务的关系

- `清洗聚合与判定规则_v0.1.yaml`包含程序读取的参数和中文业务约束；中文condition不会自行执行，须由代码实现，语义职责须转为明确AI任务。
- `输出数据契约_v0.1.yaml`约定字段与枚举，不是Agent执行计划，也不是已覆盖所有接口的统一运行时校验器。当前校验分散在代码，迁移时须逐字段对照。
- Codex搜索只接收检索指令、查询词和搜索结果Schema；Codex事件研判接收通过基础准入的事件、来源证据、允许品牌、风险标签和输出Schema。两者均不直接读取完整YAML，由程序按任务注入必要约束，不让Agent自行决定审批、删除数据或调整时间窗。

### 3.1.4 AI任务职责与禁止事项

必须由AI分析／生成承担的内容（当前接通情况以上表为准）：
...
- 不配置定时采集；由管理员按测试计划手工触发17条基础查询和双路公开搜索。
- 对有效行业事件执行9品牌关系验证。
- 生成任务草案但不下发。
...
- 管理员可在配置管理中新增、编辑、停用或删除品牌；被查询或域名规则引用的品牌不得直接删除，应先解除引用。
...
- `来源平台字典_v0.1.yaml`是来源能力与识别规则的完整配置源：采集器能力为只读技术信息；平台定义和域名规则允许管理员增删改。页面必须展示字典中的全部平台定义，不只展示当前有数据的平台。
- 平台被域名规则或采集器指标能力引用时不得直接删除；`unknown`和`other_website`为兜底项，不允许删除。
...
- 查询新增、停用或修改必须记录版本和原因。
- 管理员可新增、编辑、启停或删除查询；品牌查询必须引用已登记品牌。完整运行任务数实时读取启用查询，不把配置编号拼入真实搜索词。

## 6.4 自动采集

- 自动采集默认停止，只有管理员可开启、停止或修改周期；访客只读。
- 周期按小时配置，范围1—168小时，当前默认值为3小时。
- 开启或修改周期后，从配置时刻起等待一个完整周期；停止后立即取消下次计划，但不强行中断已经开始的批次。
- 自动采集与手工完整运行共用3小时冷却；周期短于3小时时，到期任务顺延至下一允许时间，不并发或绕过冷却。
- 自动批次与手工批次使用同一查询目录、双路采集、来源清洗、事件聚合、幂等、失败记录和审计规则。
- 页面展示当前状态、周期、下次执行时间、最近自动批次；服务重启后从持久化配置恢复，不重复创建同一到期任务。
...
可核验发布时间超过采集时刻前72小时的旧页面进入无效日志，不进入运营候选。缺失或未来日期也应排除并留痕；验收覆盖正文头部旧日期与供应商新日期冲突、正文叙事日期不能当发布日期、多工具发现记录保留及中文风险标签一致性。
