...
同一批次内同一规范化URL只保留一条`source_item`，但每一次由哪个工具、哪个查询、何时搜索发现，必须分别写入`source_discovery`。工作台独立展示来源平台、搜索来源（豆包／Codex，多路命中全部显示）、正文／搜索摘要、内容发布时间和搜索获取时间；正文／摘要截取三行，详情展示返回内容及发布时间依据。支持按两类时间分别筛选。
