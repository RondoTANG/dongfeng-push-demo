(function () {
  ...
  function eventReview(event) {
    var current = ['relevant_event_clue', 'brand_content_opportunity'].indexOf(event.event_status) >= 0 ? event.event_status : 'relevant_event_clue';
    ...
    var drawer = UI.openDrawer({
      ...
      body: '<div class="review-form" data-event-review-form>' +
        '<div class="review-summary form-field full"><strong>先确认事件是否可进入行动决策</strong><p>“通过／驳回”是最终审核结果；事实结论用于说明通过的原因，但不会自动生成作业。只有审核通过后，由运营勾选具体行动方向，系统才生成相应草案。</p></div>' +
        '<div class="form-field"><label>审核结果</label><select class="form-control" name="review_result"><option value="approved">通过并选择行动</option><option value="rejected">驳回，不生成草案</option></select></div>' +
        '<div class="form-field" data-approved-only><label>通过后的事实结论</label><select class="form-control" name="event_status">' +
          [['relevant_event_clue','事件事实成立，可考虑响应'],['brand_content_opportunity','品牌内容机会，建议响应']].map(function (item) { return '<option value="' + item[0] + '"' + (current === item[0] ? ' selected' : '') + '>' + item[1] + '</option>'; }).join('') + '</select></div>' +
        ...
        '<div class="form-field full"><label>风险说明</label><textarea class="form-control" name="risk_summary">' + AppCommon.escapeHtml((event.risk_tags || []).length ? '存在风险标签：' + (event.risk_labels || event.risk_tags).join('、') : '未发现需要阻断草案生成的明确风险，仍需遵守证据边界。') + '</textarea></div>' +
        '<div class="form-field full" data-approved-only><label>本次行动方向</label><div class="action-paths"><label><input type="checkbox" name="action_path" value="original_growth" checked><span><strong>原创增长</strong><small>围绕事件生成完整原创作业要求；实际发布后继续追踪后效</small></span></label><label class="' + (boostableSources.length ? '' : 'is-disabled') + '"><input type="checkbox" name="action_path" value="source_content_boost"' + (boostableSources.length ? '' : ' disabled') + '><span><strong>热点源内容加热</strong><small>直接对事件中的具体文章或视频生成点赞、正向评论等草案</small></span></label></div></div>' +
        '<div class="form-field full" data-boost-source-field data-approved-only><label>加热目标内容</label><div class="source-choice-list">' + sourceOptions + '</div></div>' +
    ...
    var reviewResult = drawer.element.querySelector('[name="review_result"]');
    var syncReviewResult = function () {
      var approved = reviewResult.value === 'approved';
      drawer.element.querySelectorAll('[data-approved-only]').forEach(function (node) { node.classList.toggle('is-hidden', !approved); });
    };
    reviewResult.addEventListener('change', syncReviewResult);
    syncBoostField();
    syncReviewResult();
    return drawer;
  }
