(function () {
  ...
  function renderContent() {
    ...
    var columns = [
      ...
      { label: '来源平台', width: '150px', render: function (row) { return '<strong>' + AppCommon.escapeHtml(platformNames[row.source_platform] || row.source_platform) + '</strong><div class="cell-sub">' + AppCommon.escapeHtml(row.source_site_name || row.domain || '未识别站点') + '</div>'; } },
      { label: '搜索来源', width: '125px', render: function (row) { return '<div class="provider-row">' + (row.discovered_by && row.discovered_by.length ? row.discovered_by : [row.retrieved_by]).map(function (item) { return '<span class="mini-tag">' + AppCommon.escapeHtml(AppCommon.providerName(item)) + '</span>'; }).join('') + '</div>'; } },
      { label: '正文／搜索摘要', width: '270px', render: function (row) { return '<div class="line-clamp-3 clue-excerpt">' + AppCommon.escapeHtml(row.snippet || '未返回正文摘要') + '</div>'; } },
